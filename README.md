...
[![VRAM: 12GB Tested](https://img.shields.io/badge/VRAM-12GB%20Target-success.svg)](#-benchmarks-9-faixas-produzidas)
[![Audio Demos](https://img.shields.io/badge/Audio%20Demos-9%20Tracks%20(320k)-brightgreen.svg)](#-demonstrações-em-áudio-mp3-320-kbps)
...
│                                      │                                      │
│               [Fade-Out em Cosseno Analógico Inteligente (5s)]              │
│                                      │                                      │
│               Resultado: Áudio Master (WAV Estéreo 44.1 kHz)                │
└─────────────────────────────────────────────────────────────────────────────┘
```

1. **Quantização NF4 de 4 bits em Memória (BitsAndBytes):** Converte recursivamente as 253 camadas lineares do Qwen-8B para `BitsAndBytesLinear4bit`, reduzindo os pesos de 8.76 GB para ~4.40 GB de VRAM.
2. **KV-Cache Nativo em BF16:** Preserva as chaves e valores de atenção em `bfloat16` puro, eliminando o ruído inicial e garantindo dicção cristalina desde a primeira sílaba cantada.
3. **Transição de VRAM em Duas Fases (Two-Phase Swapping):** Executa o LM e RVQ na GDDR6, descarrega-os completamente após a geração semântica e carrega o DiT de 2.4B diretamente na VRAM, sem paginação de camadas via PCIe.
4. **Finalização com Fade-Out Analógico:** Analisa a energia da cauda do áudio e aplica atenuação em cosseno nos últimos 5 segundos se necessário, eliminando cortes secos.

---

## 📊 Benchmarks: 9 Faixas Produzidas

Todas as faixas foram geradas de forma autônoma em uma **NVIDIA GeForce RTX 3060 (12GB GDDR6)**:

| # | Título da Faixa | Gênero / Identidade Sonora | Duração | Tempo de Produção | Taxa Semântica | VRAM Pico | Arquivo Master |
| :-: | :--- | :--- | :-: | :-: | :-: | :-: | :--- |
...
Listen to all 9 full-length master tracks directly in your browser, rendered autonomously on a consumer **NVIDIA RTX 3060 (12GB VRAM)**:
...
| **9** | **Na Manha do Gato** | Brazilian Samba-Rap / Rio Hip-Hop | 3m 09s | [▶️ Listen to Track](demos/na_manha_do_gato_master.mp3) |
